...
> - ⏳ **Rollout (in review, not yet deployed):** the enforcement, the `isMandatory`/`isHidden` flags in Context (Lite), and the `details.field` population all ship together in the `dutify-pm` change (owner: Dutify Cloud, peer #923). Until it merges + deploys, **degrade gracefully**: production may still silently accept a missing mandatory field, `isMandatory` may be absent (treat as `false`), and `details.field` may be absent (fall back to parsing the field name from `message`). The `message` wording is stable across the rollout.
