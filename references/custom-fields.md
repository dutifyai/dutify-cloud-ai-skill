...
> ⚠️ **Mandatory custom fields.** A list can mark custom fields *required/mandatory*. Each custom field in the **Context (Lite)** bundle carries `isMandatory` (boolean) and `isHidden`. Before creating or updating a task, populate every field where `isMandatory=true` for the target list. The API enforces it: a write that leaves a mandatory field empty is rejected with **HTTP 400, `errorCode: "VALIDATION_ERROR"`**, and a `message` naming the missing field (e.g. `Mandatory custom field 'Priority' requires a value`) — **parse the field name from `message`**. Fix and retry — do **not** interrogate the user as a substitute for populating the value.
> - ⏳ **`details.field` is planned, not yet populated.** Unlike other lite validation errors, this one currently carries the field name **only in `message`**. The structured details map (`details.field` = display name) is pending a `ServiceResult`/`ResourceHelper` extension — until it ships, read the field name from `message`, don't rely on `details.field`.
