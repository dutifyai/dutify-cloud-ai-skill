...
- **`email`, `phone`, `website`, `location`** — each accepts a plain string (the most-important field — email address, phone number, URL, address respectively) **or** a structured object with extra fields. Read the catalog's request schema for the object form when you need it (`{email, displayName, verified}` for `email`; `{number, countryCode, extension}` for `phone`; `{url, title, description}` for `website`; `{address, city, state, country, zipCode, latitude, longitude}` for `location` with the constraint that `latitude` and `longitude` are paired and must both be set or both omitted).
  - **`email` supports MULTIPLE values — but only opt-in per field**, via `visualSettings.allowsMultiple` (defaults to **false** when `visualSettings` or the flag is absent).
    - **`allowsMultiple: true`** → pass a JSON **array**; elements may be email strings and/or objects (`{email, displayName, verified}`), and mixing the two in one array is fine. Each element is validated with the normal single-email rules (nested arrays / numbers are rejected). **Max 200 elements** — a longer array returns 400 `VALIDATION_ERROR` with `Array value exceeds maximum size of 200 elements` (hardcoded, not configurable). Multi values display joined with `", "`.
    - **`allowsMultiple: false`/absent (the default)** → single-value only: a plain string `"a@b.com"` or an object `{"email": "a@b.com"}`. An **array is rejected** with `Email field value must be a string or object`.
    - ⚠️ **If you hit that rejection, the fix is to enable `allowsMultiple` on the field — not to flatten the value.** Do **not** use a comma-separated string as a workaround: `"a@b.com, c@d.com"` is treated as one address and fails email-format validation.
    - ⚠️ **`allowsMultiple` cannot be set from the lite surface.** `POST/PUT /v1/custom-fields/lite` bind `LiteCustomFieldCreation/UpdateRequest`, which have no `visualSettings` — the lite→rich mapping copies only name/type/description/emoji/color/options/isPrivate and never sets it. To turn it on, use a **rich** scope endpoint (`/v1/{workspaces|spaces|folders|lists}/{id}/custom-fields`), same as the mandatory/hidden toggles. Once enabled, set values through whichever value endpoint you normally use.
    - **`phone` / `website` / `location` are still single-value** — those handlers have no `allowsMultiple` support at all, so arrays are always rejected for them.
